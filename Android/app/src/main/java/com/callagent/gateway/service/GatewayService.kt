...
class GatewayService : Service() {
    ...
    private fun startGateway(intent: Intent?) {
        ...
        val prefs = getSharedPreferences("gateway", MODE_PRIVATE)
        val server = intent?.getStringExtra(EXTRA_SERVER) ?: prefs.getString("server", "sip.example.com") ?: ""
    ...
    private fun initSipClient(gen: Int) {
        ...
        // TLS protects signalling only: REGISTER and INVITE.  RTP is
        // deliberately left alone, so call audio is no more or less private
        // than it was on UDP.
        val prefs = getSharedPreferences("gateway", MODE_PRIVATE)
     ...
     *  foreground does not rewrite what the user sees. */
